...
|---|---|---|---|---|---|---|---|---|
| Public | Brand, title, description, canonical | COMPLETE | `okwook-brand-domain` V2 | V3 clean-browser metadata and render PASS | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_LOCAL_MODE | None in no-call scope | P2 |
| Public | Home page and footer | COMPLETE | Brand assertions V2 | V3 fresh context/profile PASS | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_LOCAL_MODE | None in no-call scope | P2 |
| Public | Login entry | COMPLETE | `login-user-entry`, `auth-login-route-context` V2 | V3 fresh context/profile PASS | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_LOCAL_MODE | Real delivery adapter | P1 |
| Public | Registration entry | COMPLETE | `register-entry`, consent tests V2 | V3 fresh context/profile PASS | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_LOCAL_MODE | Real delivery adapter | P1 |
| Public | 404 route | NOT_IMPLEMENTED | V0 | V0 | N/A | NON_BLOCKING_GAP | Add catch-all route | P3 |
| Public | Install redirect after initialization | COMPLETE | Production/init tests V2 | V3 clean profile redirects safely to `/` | N/A | DEPLOY_READY_LOCAL_MODE | None in no-call scope | P2 |
| Auth | Phone/email code login | COMPLETE | Login, consent, session tests V2 | V3 clean login UI rendered | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_FEATURE_DISABLED | Real delivery adapter | P1 |
| Auth | Auto-registration | COMPLETE | `register-entry` V2 | V3 clean registration UI rendered | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_FEATURE_DISABLED | Real delivery adapter | P1 |
...
| Account | Membership and points display | COMPLETE | Payment parked test V2 | V3 protected route redirects to user login context | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_FEATURE_DISABLED | Authenticated fixture browser run | P1 |
| Account | Generation history and detail | COMPLETE | History safety test V2 | V3 protected route redirects to user login context | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_LOCAL_MODE | Authenticated fixture browser run | P2 |
...
| Generate | Form and model selection | COMPLETE | No-call guidance tests V2 | V3 fresh context/profile PASS | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_FEATURE_DISABLED | Real Provider validation | P1 |
...
| Payment | Plans and recharge UI | COMPLETE | Payment parked V2 | V3 public entry rendered in clean contexts | EXTERNAL_VALIDATION_DEFERRED | DEPLOY_READY_FEATURE_DISABLED | Authenticated flow coverage | P1 |
...
Counts: 50 independently assessed features. COMPLETE 38, PARTIAL 7, SCAFFOLD_ONLY 2, NOT_IMPLEMENTED 3, DEPRECATED 0, NOT_APPLICABLE 0. The verification-level numbers are evidence references across feature rows, so one feature can hold V2 automated evidence alongside V3 clean-browser evidence. No V4 or V5 evidence is claimed in this audit.
